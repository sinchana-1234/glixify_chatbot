...
class HealthProgressBase(BaseTool):
    ...
    def _build_chart(self, metric: str, glucose_daily: list, data: dict, from_date: str, to_date: str) -> dict:
        if metric == "hba1c":
            ...
            return {
                ...
                "series": [
                    {"name": "eHbA1c (%)", "values": [d["estimatedHba1c"] for d in valid]},
                ],
                "color": "#4a90e2",
        ...
        if metric == "fbs":
            ...
            return {
                ...
                "x_labels": [d["glucoseDate"] for d in valid],
                "series": [
                    {"name": "Fasting BS (mg/dL)", "values": [d["fbs"] for d in valid]},
                ],
                "color": "#e0576a",
                "stats": {"avg_tir_pct": None, "avg_bp": None, "avg_hba1c": None}
            }
    ...
    def _summarize(self, metric: str, glucose_daily: list, data: dict) -> dict:
        if metric == "hba1c":
            valid = [d["estimatedHba1c"] for d in glucose_daily if d.get("estimatedHba1c") is not None]
            if not valid:
                return {"days_count": 0}
            return {
                "days_count": len(valid),
                "avg_hba1c_pct": round(sum(valid) / len(valid), 2),
                "max_hba1c_pct": round(max(valid), 2),
                "min_hba1c_pct": round(min(valid), 2),
            }
        if metric == "fbs":
            valid = [d["fbs"] for d in glucose_daily if d.get("fbs") is not None]
            if not valid:
                return {"days_count": 0}
            return {
                "days_count": len(valid),
                "avg_fbs": round(sum(valid) / len(valid), 1),
                "max_fbs": round(max(valid), 1),
                "min_fbs": round(min(valid), 1),
            }
        if metric == "tir":
            return {
        ...
        if metric == "heart_rate":
            ...
            return {
                ...
                "max_hr_bpm": max(d.get("highHeartRate", 0) for d in hr),
                "min_hr_bpm": min(d.get("lowHeartRate", 0) for d in hr),
            }
        if metric == "stress_hrv":
            stress = data.get("stress") or []
            hrv = data.get("hrv") or []
            return {
                "days_count": max(len(stress), len(hrv)),
                "avg_stress_pct": round(sum(d.get("average", 0) for d in stress) / len(stress), 1) if stress else None,
                "avg_hrv_ms": round(sum(d.get("average", 0) for d in hrv) / len(hrv), 1) if hrv else None,
            }
        return {}
    ...
    def _run_for_metric(self, metric: str, patient_id: Optional[int] = None,
        ...
        if not glucose_daily:
            if user_specified_range:
                ...
                notice = (
                    f"There is no data for patient {patient_id} "
                ...
                if not probe_glucose:
                    ...
                    object.__setattr__(self, 'last_chart_data', None)
                    return json.dumps({
                        "patient_id": patient_id, "status": status, "notice": notice,
                    })
                ...
                notice = (
                    f"No data in the last 7 days ({from_date} to {to_date}). "
        ...
        if metric == "tir":
            ...
            base_payload["heart_rate" if chart_data is None else "summary"] = \
                (data.get("hr") or []) if chart_data is None else self._summarize(metric, glucose_daily, data)
        elif metric == "stress_hrv":
            if chart_data is not None:
                base_payload["summary"] = self._summarize(metric, glucose_daily, data)
            else:
                base_payload["stress"] = data.get("stress") or []
                base_payload["hrv"] = data.get("hrv") or []
        else:
            base_payload.update({
...
_COMMON_TAIL = (
    ...
    "'ambiguous_name', 'not_found', 'no_data_in_range' (explicit dates given, "
    "none found — say so plainly, never substitute other dates), "
    "'fallback_most_recent' (no dates given, none in the last 7 days, showing "
    "older data instead), or 'no_data_at_all'. Always relay the 'notice' field "
    "verbatim when present. A chart is ALREADY shown automatically when data "
    "is found — do NOT write out a table or list of daily values yourself; "
    "give only a brief 1-3 sentence summary."
)
